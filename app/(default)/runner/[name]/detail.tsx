"use client"
import { Runner } from "@/app/types/runner"
...
import { deregisterRunner } from "@/app/api/runner"
import { ExclamationTriangleIcon, CheckCircleIcon } from "@heroicons/react/24/outline"

export default function RunnerDetail(res) {
  ...
  const handleConfirmYes = async () => {
    const res = await deregisterRunner([runner.runnerId])
  ...
  const handleApiCall = async (res: RunnerApiResponse) => {
  ...
  return (
    <div className='flex-none bg-gray-100 px-12 py-12 bg-gray-800'>
      <h2 className='flex dark:text-white text-3xl py-6 font-bold'>{`Runner > ${runner.runnerName}`}</h2>
      ...
      {showConfirm && (
        <PopupModal text={`Do you want to register ${runner.runnerName}?`} open yesLabel='Yes' noLabel='No' onYes={handleConfirmYes} onNo={handleConfirmNo} />
      )}
          ...
          <div>
            ...
            <div className=''>
              <div className='flex items-center'>
                {getStatusIcon(runner.status)} {runner.status}
              </div>
            </div>
          </div>
          <div>
            <div className='font-bold text-lg'>Created at</div>
            <div className=''>{runner.createdAt}</div>
          </div>
      ...
      <div className='flex-grow x p-4 border border-white max-w-7xl dark:text-white'>
        {runner.tags && Object.keys(runner.tags).length > 0 && (
          <>
            <div>
              {Object.entries(runner.tags).map(([key, value]) => (
